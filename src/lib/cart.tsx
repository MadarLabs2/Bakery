...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  const refresh = useCallback(async () => {
    ...
    const { data } = await supabase
      .from("cart_items")
      .select("id, product_id, quantity, product:products(id, name_en, name_he, name_ar, price, image_url)")
      .eq("cart_id", cartId);
  ...
  return (
    <Ctx.Provider value={{ items, count, subtotal, loading, addToCart, updateQty, remove, clear, refresh }}>
      {children}
    </Ctx.Provider>
  );
}
