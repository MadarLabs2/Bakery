...
function AdminDashboard() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setStats({ orders: o.count ?? 0, products: p.count ?? 0, bestSellers: b.count ?? 0, revenue });
      setRecent(r.data ?? []);
    });
  }, []);
          ...
          {recent.map(o => (
            <div key={o.id} className="flex items-center justify-between border-b py-2 last:border-0">
              <div>
                ...
                <div className="text-xs text-muted-foreground">#{o.id.slice(0, 8)} · {format(new Date(o.created_at), "PP")}</div>
              </div>
