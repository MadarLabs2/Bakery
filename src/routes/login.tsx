    ...
    <div className="container mx-auto flex min-h-[70vh] items-center justify-center px-4 py-10">
      <div className="w-full max-w-md rounded-2xl border bg-card p-8 shadow-lg">
        ...
        <form onSubmit={submit} className="mt-6 space-y-4">
          ...
          <Button type="submit" className="w-full" size="lg" disabled={busy}>{t("login")}</Button>
        </form>
        <p className="mt-4 text-center text-sm text-muted-foreground">
          {t("register")}? <Link to="/register" className="text-primary hover:underline">{t("register")}</Link>
        </p>
      </div>
    </div>
  );
