...
import { LayoutDashboard, Package, FolderTree, ShoppingCart, Tag, Mail, LogOut, Wheat } from "lucide-react";
  ...
  return (
    <div className="flex min-h-screen">
      <aside className="hidden md:flex w-64 flex-col border-r bg-card p-4">
        <Link to="/" className="mb-6 flex items-center gap-2 font-display text-lg font-bold text-primary">
      ...
      <main className="flex-1 overflow-auto bg-background">
        <div className="md:hidden border-b bg-card p-3 overflow-x-auto">
          <div className="flex gap-2">
            {links.map(l => (
              <Link key={l.to} to={l.to as any} className="whitespace-nowrap rounded-md border px-3 py-1.5 text-xs">{l.label}</Link>
            ))}
          </div>
        </div>
        <Outlet />
