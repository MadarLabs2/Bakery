...
function AdminCategories() {
  ...
  const save = async () => {
    ...
    toast.success("Saved"); setOpen(false); setEditing(empty); load();
  };
    ...
    <div className="p-6 md:p-8 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild><Button onClick={() => setEditing(empty)}><Plus className="h-4 w-4" /> New</Button></DialogTrigger>
          <DialogContent>
            ...
            <div className="space-y-3">
              ...
              <div className="grid gap-3 sm:grid-cols-3">
                ...
                <div><Label>HE</Label><Input value={editing.name_he} onChange={e => setEditing({ ...editing, name_he: e.target.value })} /></div>
                <div><Label>AR</Label><Input value={editing.name_ar} onChange={e => setEditing({ ...editing, name_ar: e.target.value })} /></div>
              </div>
              <div><Label>Order</Label><Input type="number" value={editing.display_order} onChange={e => setEditing({ ...editing, display_order: e.target.value })} /></div>
      ...
      <div className="rounded-2xl border bg-card overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-muted/50 text-left"><tr><th className="p-3">Slug</th><th>EN</th><th>HE</th><th>AR</th><th>Order</th><th></th></tr></thead>
          <tbody>
            {cats.map(c => (
              <tr key={c.id} className="border-t">
                <td className="p-3 font-mono text-xs">{c.slug}</td>
                <td>{c.name_en}</td><td>{c.name_he}</td><td>{c.name_ar}</td><td>{c.display_order}</td>
                <td className="text-right pr-3">
                  <Button variant="ghost" size="icon" onClick={() => { setEditing(c); setOpen(true); }}><Edit className="h-4 w-4" /></Button>
