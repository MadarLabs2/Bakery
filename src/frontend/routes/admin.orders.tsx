...
const TABS: { id: TabId; labelKey: DictKey; match: (status: string) => boolean }[] = [
...
function tabCount(orders: any[], tab: TabId): number {
  return orders.filter((o) => orderMatchesTab(String(o.order_status), tab)).length;
}

function dayBucket(orders: any[], day: Date) {
  return orders.filter((o) => isSameDay(new Date(o.created_at), day));
}
    ...
    >
      ...
      <div className="flex min-w-0 flex-1 items-center gap-3 px-3 py-3.5">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 text-sm font-semibold text-neutral-900">{o.customer_name}</p>
        ...
        {TABS.map((def) => {
          ...
          return (
            ...
            >
              {t(def.labelKey)}
              {def.id !== "all" ? (
                <span className={cn("ms-1 tabular-nums opacity-80", active ? "text-white/90" : "text-stone-500")}>
                  ({count})
                </span>
              ) : null}
            </button>
                  ...
                  <tbody>
                    {filtered.map((o) => (
                      ...
                      >
                        ...
                        <td className="whitespace-nowrap px-4 py-3 text-muted-foreground">
                          {deliveryLabel(o.delivery_method, t)} · {paymentLabel(o.payment_method, t)}
                        </td>
                        <td className="whitespace-nowrap px-4 py-3 font-semibold tabular-nums text-[#1B4332]" dir="ltr">
                          ₪{Number(o.total_amount).toFixed(2)}
                        </td>
            ...
            <>
              ...
              <div className="max-h-[min(70dvh,32rem)] space-y-0 overflow-y-auto px-5 py-4 text-sm">
                <section className="rounded-xl border border-stone-200/80 bg-white p-3 shadow-sm">
                  ...
                  <p className="mt-1 font-semibold text-neutral-900">{selected.customer_name}</p>
                  <div className="mt-2 space-y-1 text-sm text-neutral-600">
                    <p>
                      <a href={`tel:${String(selected.customer_phone ?? "").replace(/\s/g, "")}`} className="text-[#2f6a4f] hover:underline">
                        {selected.customer_phone}
                      </a>
                    </p>
                    <p>
                      <a href={`mailto:${selected.customer_email}`} className="break-all text-[#2f6a4f] hover:underline">
                ...
                <section className="mt-3 rounded-xl border border-stone-200/80 bg-white p-3 shadow-sm">
                  ...
                  <ul className="mt-2 divide-y divide-stone-100">
                    {(selected.items ?? []).map((it: any) => {
                      const rawUrl = it.product?.image_url as string | undefined;
                      const thumb = rawUrl ? resolveImage(rawUrl) : null;
                      return (
                        <li key={it.id} className="flex gap-3 py-2.5 first:pt-0">
                          ...
                          <div className="min-w-0 flex-1">
                            <p className="font-medium leading-snug text-[#2f6a4f]">{it.product_name}</p>
                            <p className="mt-0.5 text-xs tabular-nums text-neutral-500" dir="ltr">
                              {it.quantity} × ₪{Number(it.product_price).toFixed(2)}
                            </p>
                          </div>
                          <span className="shrink-0 text-sm font-semibold tabular-nums text-neutral-900" dir="ltr">
