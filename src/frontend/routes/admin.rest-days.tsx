...
import { RestDayDateRangeSelector } from "@/frontend/components/admin/RestDayDateRangeSelector";
import { toast } from "sonner";
import { cn } from "@/frontend/lib/utils";

export const Route = createFileRoute("/admin/rest-days")({ component: AdminRestDaysPage });
...
function AdminRestDaysPage() {
  ...
  const load = async () => {
    setLoading(true);
    const result = await fetchAdminRestDays();
    if (!result.ok) {
  ...
  const handleDelete = async (id: string) => {
    const result = await deleteRestDay(id);
    if (!result.ok) {
  ...
  const handleToggle = async (row: RestDayRow) => {
    const result = await updateRestDayStatus(row.id, !row.is_active);
    if (!result.ok) {
      toast.error(result.message);
      return;
    }
