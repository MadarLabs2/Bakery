...
-- Project: Al-Nour Bakery — pickup/delivery time slots
--
-- Requires: fulfillment_available_days (run apply_fulfillment_available_days.sql first)
-- Same content as: supabase/migrations/20260619120000_fulfillment_time_slots.sql
-- =============================================================================

-- ── 1. Time slots per weekday ────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS public.fulfillment_time_slots (
  ...
  UNIQUE (fulfillment_type, day_of_week, time_value)
);

CREATE INDEX IF NOT EXISTS idx_fulfillment_time_slots_lookup
  ON public.fulfillment_time_slots (fulfillment_type, day_of_week, time_value)
  WHERE enabled = true;

DROP TRIGGER IF EXISTS trg_fulfillment_time_slots_updated_at ON public.fulfillment_time_slots;
CREATE TRIGGER trg_fulfillment_time_slots_updated_at
  BEFORE UPDATE ON public.fulfillment_time_slots
  FOR EACH ROW
  EXECUTE FUNCTION public.set_updated_at();

ALTER TABLE public.fulfillment_time_slots ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Public read fulfillment time slots" ON public.fulfillment_time_slots;
CREATE POLICY "Public read fulfillment time slots"
  ON public.fulfillment_time_slots FOR SELECT
  USING (true);

DROP POLICY IF EXISTS "Admins manage fulfillment time slots" ON public.fulfillment_time_slots;
CREATE POLICY "Admins manage fulfillment time slots"
  ON public.fulfillment_time_slots FOR ALL
  USING (public.is_admin())
  WITH CHECK (public.is_admin());

-- Default pickup: Tue 10/12/14, Fri 09/11/13
INSERT INTO public.fulfillment_time_slots (fulfillment_type, day_of_week, time_value, enabled)
VALUES
  ('pickup', 2, '10:00', true),
  ('pickup', 2, '12:00', true),
  ('pickup', 2, '14:00', true),
  ...
  ('pickup', 5, '11:00', true),
  ('pickup', 5, '13:00', true)
ON CONFLICT (fulfillment_type, day_of_week, time_value) DO NOTHING;

-- Default delivery: Tue 15/17, Fri 10/12
INSERT INTO public.fulfillment_time_slots (fulfillment_type, day_of_week, time_value, enabled)
VALUES
  ('delivery', 2, '15:00', true),
  ('delivery', 2, '17:00', true),
  ('delivery', 5, '10:00', true),
  ('delivery', 5, '12:00', true)
ON CONFLICT (fulfillment_type, day_of_week, time_value) DO NOTHING;

-- ── 2. Order column for selected time ────────────────────────────────────────
ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS selected_fulfillment_time TIME;

-- ── 3. create_order_secure — validate date + time slot ───────────────────────
...
CREATE OR REPLACE FUNCTION public.create_order_secure(
  ...
  p_fulfillment_time            TEXT DEFAULT NULL
)
RETURNS JSON
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF p_fulfillment_time IS NULL OR trim(p_fulfillment_time) = '' THEN
    RAISE EXCEPTION 'ERR_MISSING_FULFILLMENT_TIME';
  END IF;

  BEGIN
    v_time := trim(p_fulfillment_time)::TIME;
  EXCEPTION WHEN OTHERS THEN
    RAISE EXCEPTION 'ERR_INVALID_FULFILLMENT_TIME';
  END;

  IF NOT EXISTS (
    SELECT 1
    FROM   public.fulfillment_time_slots fts
    WHERE  fts.fulfillment_type = p_delivery_method
      AND  fts.day_of_week      = p_fulfillment_day_of_week
      AND  fts.time_value       = v_time
      AND  fts.enabled          = true
  ) THEN
    RAISE EXCEPTION 'ERR_FULFILLMENT_TIME_NOT_AVAILABLE';
  END IF;
  ...
  BEGIN
    INSERT INTO public.orders (
      ...
      selected_fulfillment_date, selected_fulfillment_day_of_week,
      selected_fulfillment_label, selected_fulfillment_time
    ) VALUES (
...
REVOKE ALL ON FUNCTION public.create_order_secure(
  TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, DATE, INT, TEXT, TEXT
) FROM PUBLIC;
