...
-- Project: Al-Nour Bakery — delivery places with per-area pricing
-- Same content as: supabase/migrations/20260621120000_delivery_places.sql
-- Prerequisite: public.is_bakery_rest_day() from apply_bakery_rest_days.sql (or included below)
-- =============================================================================

-- Delivery places — per-area delivery pricing

CREATE TABLE IF NOT EXISTS public.delivery_places (
...
CREATE INDEX IF NOT EXISTS idx_delivery_places_active_sort
  ON public.delivery_places (sort_order, name_en)
  WHERE is_active = true;

DROP TRIGGER IF EXISTS trg_delivery_places_updated_at ON public.delivery_places;
CREATE TRIGGER trg_delivery_places_updated_at
  BEFORE UPDATE ON public.delivery_places
  FOR EACH ROW
  EXECUTE FUNCTION public.set_updated_at();

ALTER TABLE public.delivery_places ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Public read active delivery places" ON public.delivery_places;
CREATE POLICY "Public read active delivery places"
  ON public.delivery_places FOR SELECT
  USING (is_active = true);

DROP POLICY IF EXISTS "Admins manage delivery places" ON public.delivery_places;
CREATE POLICY "Admins manage delivery places"
  ON public.delivery_places FOR ALL
  USING (public.is_admin())
  WITH CHECK (public.is_admin());

ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS delivery_place_id UUID REFERENCES public.delivery_places(id),
  ADD COLUMN IF NOT EXISTS delivery_place_name TEXT;

-- Rest-day helper (no-op if bakery_rest_days table is empty / missing rows)
...
-- Recreate create_order_secure with delivery place pricing (includes rest-day validation)
...
DROP FUNCTION IF EXISTS public.create_order_secure(TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, DATE, INT, TEXT, UUID);
...
CREATE OR REPLACE FUNCTION public.create_order_secure(
  ...
  p_delivery_place_id           UUID DEFAULT NULL
)
RETURNS JSON
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_dow                   INT;
  v_place                 RECORD;
  v_delivery_place_name   TEXT           := NULL;
BEGIN
  ...
  IF p_delivery_method = 'delivery' THEN
    ...
    IF NOT EXISTS (
      SELECT 1 FROM public.delivery_places WHERE is_active = true
    ) THEN
      RAISE EXCEPTION 'ERR_DELIVERY_UNAVAILABLE';
    END IF;

    IF p_delivery_place_id IS NULL THEN
      RAISE EXCEPTION 'ERR_MISSING_DELIVERY_PLACE';
    END IF;

    SELECT id, name_he, name_ar, name_en, price
    INTO   v_place
    FROM   public.delivery_places
    WHERE  id = p_delivery_place_id
      AND  is_active = true
    LIMIT  1;

    IF NOT FOUND THEN
      RAISE EXCEPTION 'ERR_INVALID_DELIVERY_PLACE';
    END IF;

    v_delivery_fee := v_place.price;
    v_delivery_place_name := CASE v_locale
      WHEN 'he' THEN v_place.name_he
      WHEN 'ar' THEN v_place.name_ar
      ELSE v_place.name_en
    END;
  END IF;
  ...
  BEGIN
    INSERT INTO public.orders (
      ...
      delivery_place_id, delivery_place_name
    ) VALUES (
      ...
      CASE WHEN p_delivery_method = 'delivery' THEN p_delivery_place_id ELSE NULL END,
      CASE WHEN p_delivery_method = 'delivery' THEN v_delivery_place_name ELSE NULL END
    )
...
REVOKE ALL ON FUNCTION public.create_order_secure(
  TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, DATE, INT, TEXT, UUID
) FROM PUBLIC;
